  ...
  async function fetchAndSetHomework() {
    try {
      const fetchHomeworkResult = await API.graphql(
        graphqlOperation(homeworkByStudentAndAssignment, {
      ...
      reportError(
        error,
        `We're sorry. There was an error while attempting to fetch your current assignment. Please wait a moment and try again.`,
      );
    }
  }
